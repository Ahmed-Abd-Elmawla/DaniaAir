...
@section('content')
                        ...
                        <tr class="align-middle table-row-height">
                            ...
                            <td>
                                <div class="ul-widget-app__profile-pic">
                                    <img class="rounded-circle" src="{{ $admin->image }}" width="60" height="60">
                                </div>
                            </td>
                            <td>{{ $admin->name }}</td>
                            <td>{{ $admin->email }}</td>
                            ...
                            <td class="p-0">
                                <div class="actions-cell">
                                    <a href="#" class="btn btn-link"
                                        data-bs-title="{{ __('dashboard.admin.update') }}"
                                        onclick="openModal('edit',{{ json_encode($admin) }})">
                                        <i class="bi bi-pencil-square fs-4 text-info"></i>
                                    </a>
                                    <a href="#" class="btn btn-link"
                                        data-bs-title="{{ __('dashboard.admin.delete') }}"
                                        onclick="deleteElement('{{ $admin->uuid }}','{{ app()->getLocale() }}/dashboard/admins')">
                    ...
                    <form class="needs-validation" id="editCreateForm" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row g-3">
                                ...
                                <!--begin::Col-->
                                <div class="col-md-6">
                                    <label for="password" class="form-label">{{ __('dashboard.layout.password') }}</label>
    ...
    <script>
        ...
        function openModal(type, data = null) {
            ...
            if (type === 'edit') {
                modalTitle.textContent = '{{ __('dashboard.admin.update') }}';
                alert.style.display = 'block'
                itemId.value = data.uuid;
                ...
                modalTitle.textContent = '{{ __('dashboard.admin.add') }}';
    ...
    <script>
        ...
        function submitForm(event, type = null) {
            ...
            if (type === 'edit') {
                action = `/${local}/dashboard/admins/update/${uuid}`;
            } else {
                action = `/${local}/dashboard/admins/store`;
            }
